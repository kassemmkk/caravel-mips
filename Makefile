...
# Custom targets for multi-peripheral user project
PROJECT_NAME = user_project_wb_wrapper
RTL_DIR = verilog/rtl
PERIPH_DIR = $(RTL_DIR)/periph
SYN_DIR = syn

# RTL files
RTL_FILES = \
$(RTL_DIR)/user_project_wb_wrapper.v \
$(RTL_DIR)/user_project.v \
$(RTL_DIR)/wb_interconnect.v \
$(PERIPH_DIR)/i3c_controller.v \
$(PERIPH_DIR)/CF_SPI_WB.v \
$(PERIPH_DIR)/CF_SPI.v \
$(PERIPH_DIR)/spi_master.v \
$(PERIPH_DIR)/EF_GPIO8_WB.v \
$(PERIPH_DIR)/EF_GPIO8.v \
$(PERIPH_DIR)/cf_util_lib.v \
$(PERIPH_DIR)/aucohl_lib.v

.PHONY: lint synth stats

lint:
@echo "Running Verilator lint check..."
verilator --lint-only --Wno-EOFNEWLINE --top-module $(PROJECT_NAME) $(RTL_FILES)
@echo "Lint check completed successfully!"

synth: $(SYN_DIR)/$(PROJECT_NAME)_synth.v

$(SYN_DIR)/$(PROJECT_NAME)_synth.v: $(RTL_FILES) $(SYN_DIR)/yosys.ys
@echo "Running Yosys synthesis..."
cd . && yosys $(SYN_DIR)/yosys.ys
@echo "Synthesis completed successfully!"

stats: $(SYN_DIR)/$(PROJECT_NAME)_synth.v
@echo "=== Project Statistics ==="
@echo "RTL Files: $(words $(RTL_FILES))"
@echo "Lines of RTL:"
@wc -l $(RTL_FILES) | tail -1
@echo ""
@echo "=== Synthesis Statistics ==="
@grep -A 30 "Number of cells:" $(SYN_DIR)/$(PROJECT_NAME)_synth.v | head -20 || echo "Run 'make synth' first"
